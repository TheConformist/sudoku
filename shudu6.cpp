...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-c") { // 处理 -c 选项
            ...
            if (i + 1 < argc) {
                ...
                int n1 = numHolesRangeMin + (numHolesRangeMax - numHolesRangeMin)/3;
                int n2 = numHolesRangeMin + 2*(numHolesRangeMax - numHolesRangeMin)/3;
                if (difficulty == 1)
                {
                    numHoles = numHolesRangeMin + rand() % (n1 - numHolesRangeMin);
                }
                else if ((difficulty == 2)
                {
                    numHoles = n1 + rand() % (n2 - n1);
                }
                else if((difficulty == 3)
                {
                    numHoles = n2 + rand() % (numHolesRangeMax - n2);
                }
                else 
                {
            ...
            if (i + 1 < argc) {
                numHolesRangeMin = std::atoi(argv[++i]);
                i++;
                numHolesRangeMax = std::atoi(argv[++i]);
                numHolesRange = numHolesRangeMax - numHolesRangeMin
                if ((numGames == 0)) {
    ...
    generator.difficulty = (int)(numHoles);
